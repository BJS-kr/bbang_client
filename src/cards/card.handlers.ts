...
function handleNormalBBang({ socket, version, sequence }: HandlerBase, user: User, targetUser: User, room: Room) {
  ...
  targetUser.character.stateInfo.setState(user.id, CharacterState.BBANG_TARGET, onBBangTimeoutTarget(damage, user, targetUser, room));
...
function handleBigBBang({ socket, version, sequence }: HandlerBase, room: Room, user: User) {
  ...
  room.users.forEach((targetUser) => {
    user.id !== targetUser.id &&
      targetUser.character.stateInfo.setState(user.id, CharacterState.BIG_BBANG_TARGET, onBBangTimeoutTarget(1, user, targetUser, room));
  });
